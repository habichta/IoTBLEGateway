...
// Project   :  BLE Gateway
// File      :  cmd_def.h
// Author    :  BlueGiga, modified by Arthur Habicht
// Compiler  :  gcc
// Brief     :  BLED112 functions to communicate with BGAPI
//==============================================================================
...
typedef void (*ble_cmd_handler)(void*,const void*); //modified by Arthur Habicht. void pointer for c++ callback relay
struct ble_header {
	uint8 type_hilen;
	uint8 lolen;
	uint8 cls;
...
extern void (*bglib_output)(int portHandle, uint8 len1, uint8* data1, uint16 len2,
		uint8* data2);
/*Modified by Arthur H. Added portHandle to parameters to choose the corresponding BLE Dongle*/
void ble_send_message(int portHandle, uint8 msgid, ...);
...
enum ble_msg_types {
	ble_msg_type_cmd = 0x00, ble_msg_type_rsp = 0x00, ble_msg_type_evt = 0x80
};
enum ble_dev_types {
...
enum ble_classes { //Modified by Arthur Habicht
	ble_cls_system = 0x00,
	ble_cls_flash = 0x01,
	ble_cls_attributes = 0x02,
	ble_cls_connection = 0x03,
	ble_cls_attclient =  0x04,
	ble_cls_sm = 0x05,
	ble_cls_gap = 0x06,
	ble_cls_hardware = 0x07,
	ble_cls_test,
	ble_cls_dfu,
	ble_cls_last
};
...
typedef enum ble_error {
	...
	ble_err_bt_bt_error_connection_terminated_by_local_host = ble_errspc_bt
			+ 22,  //Local device terminated the connection.
	...
	ble_err_bt_bt_error_connection_terminated_due_to_mic_failure = ble_errspc_bt
			+ 61, //Connection was terminated because the Message Integrity Check (MIC) failed on a received packet.
	ble_err_bt_bt_error_connection_failed_to_be_established = ble_errspc_bt
			+ 62, //LL initiated a connection but the connection has failed to be established. Controller did not receive any packets from remote end.
...
/** Define Makros that translate function to  ble_send_message(function index, parameter) **/
//** API executes function according to index and with parameter as input.
//** Indices are defined in the enums above

/*
 * Command (Send) Functions that can be used by Master. Preprocessor translates them to the actual ble_send_message function
 *protoypes also define the type of message that has to be used as a parameter.
 *
 */
...
/**Reset device**/
#define ble_cmd_system_reset(portHandle,boot_in_dfu) ble_send_message (portHandle,ble_cmd_system_reset_idx,boot_in_dfu)
/**Hello - command for testing**/
#define ble_cmd_system_hello(portHandle) ble_send_message (portHandle,ble_cmd_system_hello_idx)
/**Get device bluetooth address**/
#define ble_cmd_system_address_get(portHandle) ble_send_message (portHandle,ble_cmd_system_address_get_idx)
/**write register**/
#define ble_cmd_system_reg_write(portHandle,address,value) ble_send_message (portHandle,ble_cmd_system_reg_write_idx,address,value)
/**read register**/
#define ble_cmd_system_reg_read(portHandle,address) ble_send_message (portHandle,ble_cmd_system_reg_read_idx,address)
/**get and reset packet counters**/
#define ble_cmd_system_get_counters(portHandle) ble_send_message (portHandle,ble_cmd_system_get_counters_idx)
/**Get status from all connections**/
#define ble_cmd_system_get_connections(portHandle) ble_send_message (portHandle,ble_cmd_system_get_connections_idx)
/**Read Memory**/
#define ble_cmd_system_read_memory(portHandle,address,length) ble_send_message (portHandle,ble_cmd_system_read_memory_idx,address,length)
/**Get Device info**/
#define ble_cmd_system_get_info(portHandle) ble_send_message (portHandle,ble_cmd_system_get_info_idx)
/**Send data to endpoint, error is returned if endpoint does not have enough space**/
#define ble_cmd_system_endpoint_tx(portHandle,endpoint,data_len,data_data) ble_send_message (portHandle,ble_cmd_system_endpoint_tx_idx,endpoint,data_len,data_data)
/**Add entry to whitelist**/
#define ble_cmd_system_whitelist_append(portHandle,address,address_type) ble_send_message (portHandle,ble_cmd_system_whitelist_append_idx,address,address_type)
/**Remove entry from whitelist**/
#define ble_cmd_system_whitelist_remove(portHandle,address,address_type) ble_send_message (portHandle,ble_cmd_system_whitelist_remove_idx,address,address_type)
/**Add entry to whitelist**/
#define ble_cmd_system_whitelist_clear(portHandle) ble_send_message (portHandle,ble_cmd_system_whitelist_clear_idx)
/**Read data from endpoint, error is returned if endpoint does not have enough data.**/
#define ble_cmd_system_endpoint_rx(portHandle,endpoint,size) ble_send_message (portHandle,ble_cmd_system_endpoint_rx_idx,endpoint,size)
/**Set watermarks on both input and output side**/
#define ble_cmd_system_endpoint_set_watermarks(portHandle,endpoint,rx,tx) ble_send_message (portHandle,ble_cmd_system_endpoint_set_watermarks_idx,endpoint,rx,tx)
/**Set encryption key that will be used with aes_encrypt and aes_decrypt commands.**/
#define ble_cmd_system_aes_setkey(portHandle,key_len,key_data) ble_send_message (portHandle,ble_cmd_system_aes_setkey_idx,key_len,key_data)
/**Encrypt one block of data**/
#define ble_cmd_system_aes_encrypt(portHandle,data_len,data_data) ble_send_message (portHandle,ble_cmd_system_aes_encrypt_idx,data_len,data_data)
/**Decrypt one block of data**/
#define ble_cmd_system_aes_decrypt(portHandle,data_len,data_data) ble_send_message (portHandle,ble_cmd_system_aes_decrypt_idx,data_len,data_data)
/**Defragment persistent store**/
#define ble_cmd_flash_ps_defrag(portHandle) ble_send_message (portHandle,ble_cmd_flash_ps_defrag_idx)
/**Dump all ps keys**/
#define ble_cmd_flash_ps_dump(portHandle) ble_send_message (portHandle,ble_cmd_flash_ps_dump_idx)
/**erase all ps keys**/
#define ble_cmd_flash_ps_erase_all(portHandle) ble_send_message (portHandle,ble_cmd_flash_ps_erase_all_idx)
/**save ps key**/
#define ble_cmd_flash_ps_save(portHandle,key,value_len,value_data) ble_send_message (portHandle,ble_cmd_flash_ps_save_idx,key,value_len,value_data)
/**load ps key**/
#define ble_cmd_flash_ps_load(portHandle,key) ble_send_message (portHandle,ble_cmd_flash_ps_load_idx,key)
/**erase ps key**/
#define ble_cmd_flash_ps_erase(portHandle,key) ble_send_message (portHandle,ble_cmd_flash_ps_erase_idx,key)
/**erase flash page**/
#define ble_cmd_flash_erase_page(portHandle,page) ble_send_message (portHandle,ble_cmd_flash_erase_page_idx,page)
/**write data to user data area**/
#define ble_cmd_flash_write_data(portHandle,address,data_len,data_data) ble_send_message (portHandle,ble_cmd_flash_write_data_idx,address,data_len,data_data)
/**read data from user data area.**/
#define ble_cmd_flash_read_data(portHandle,address,length) ble_send_message (portHandle,ble_cmd_flash_read_data_idx,address,length)
/**Write to attribute database**/
#define ble_cmd_attributes_write(portHandle,handle,offset,value_len,value_data) ble_send_message (portHandle,ble_cmd_attributes_write_idx,handle,offset,value_len,value_data)
/**Read from attribute database**/
#define ble_cmd_attributes_read(portHandle,handle,offset) ble_send_message (portHandle,ble_cmd_attributes_read_idx,handle,offset)
/**Read attribute type from database**/
#define ble_cmd_attributes_read_type(portHandle,handle) ble_send_message (portHandle,ble_cmd_attributes_read_type_idx,handle)
/**Respond to user attribute read request**/
#define ble_cmd_attributes_user_read_response(portHandle,connection,att_error,value_len,value_data) ble_send_message (portHandle,ble_cmd_attributes_user_read_response_idx,connection,att_error,value_len,value_data)
/**Response to attribute_changed event where reason is user-attribute write.**/
#define ble_cmd_attributes_user_write_response(portHandle,connection,att_error) ble_send_message (portHandle,ble_cmd_attributes_user_write_response_idx,connection,att_error)
/**Send notification or indication to remote device.**/
#define ble_cmd_attributes_send(portHandle,connection,handle,value_len,value_data) ble_send_message (portHandle,ble_cmd_attributes_send_idx,connection,handle,value_len,value_data)
/**Disconnect connection, starts a disconnection procedure on connection**/
#define ble_cmd_connection_disconnect(portHandle,connection) ble_send_message (portHandle,ble_cmd_connection_disconnect_idx,connection)
/**Get Link RSSI**/
#define ble_cmd_connection_get_rssi(portHandle,connection) ble_send_message (portHandle,ble_cmd_connection_get_rssi_idx,connection)
/**Update connection parameters**/
#define ble_cmd_connection_update(portHandle,connection,interval_min,interval_max,latency,timeout) ble_send_message (portHandle,ble_cmd_connection_update_idx,connection,interval_min,interval_max,latency,timeout)
/**Request version exchange**/
#define ble_cmd_connection_version_update(portHandle,connection) ble_send_message (portHandle,ble_cmd_connection_version_update_idx,connection)
/**Get Current channel map**/
#define ble_cmd_connection_channel_map_get(portHandle,connection) ble_send_message (portHandle,ble_cmd_connection_channel_map_get_idx,connection)
/**Set Channel map**/
#define ble_cmd_connection_channel_map_set(portHandle,connection,map_len,map_data) ble_send_message (portHandle,ble_cmd_connection_channel_map_set_idx,connection,map_len,map_data)
/**Remote feature request**/
#define ble_cmd_connection_features_get(portHandle,connection) ble_send_message (portHandle,ble_cmd_connection_features_get_idx,connection)
/**Get Connection Status Parameters**/
#define ble_cmd_connection_get_status(portHandle,connection) ble_send_message (portHandle,ble_cmd_connection_get_status_idx,connection)
/**Raw TX**/
#define ble_cmd_connection_raw_tx(portHandle,connection,data_len,data_data) ble_send_message (portHandle,ble_cmd_connection_raw_tx_idx,connection,data_len,data_data)
/**Discover attributes by type and value**/
#define ble_cmd_attclient_find_by_type_value(portHandle,connection,start,end,uuid,value_len,value_data) ble_send_message (portHandle,ble_cmd_attclient_find_by_type_value_idx,connection,start,end,uuid,value_len,value_data)
/**Discover attributes by type and value**/
#define ble_cmd_attclient_read_by_group_type(portHandle,connection,start,end,uuid_len,uuid_data) ble_send_message (portHandle,ble_cmd_attclient_read_by_group_type_idx,connection,start,end,uuid_len,uuid_data)
/**Read all attributes where type matches**/
#define ble_cmd_attclient_read_by_type(portHandle,connection,start,end,uuid_len,uuid_data) ble_send_message (portHandle,ble_cmd_attclient_read_by_type_idx,connection,start,end,uuid_len,uuid_data)
/**Discover Attribute handle and type mappings**/
#define ble_cmd_attclient_find_information(portHandle,connection,start,end) ble_send_message (portHandle,ble_cmd_attclient_find_information_idx,connection,start,end)
/**Read Characteristic value using handle**/
#define ble_cmd_attclient_read_by_handle(portHandle,connection,chrhandle) ble_send_message (portHandle,ble_cmd_attclient_read_by_handle_idx,connection,chrhandle)
/**write data to attribute**/
#define ble_cmd_attclient_attribute_write(portHandle,connection,atthandle,data_len,data_data) ble_send_message (portHandle,ble_cmd_attclient_attribute_write_idx,connection,atthandle,data_len,data_data)
/**write data to attribute using ATT write command**/
#define ble_cmd_attclient_write_command(portHandle,connection,atthandle,data_len,data_data) ble_send_message (portHandle,ble_cmd_attclient_write_command_idx,connection,atthandle,data_len,data_data)
/**Send confirmation for received indication, use only if manual indications are enabled**/
#define ble_cmd_attclient_indicate_confirm(portHandle,connection) ble_send_message (portHandle,ble_cmd_attclient_indicate_confirm_idx,connection)
/**Read Long Characteristic value**/
#define ble_cmd_attclient_read_long(portHandle,connection,chrhandle) ble_send_message (portHandle,ble_cmd_attclient_read_long_idx,connection,chrhandle)
/**Send prepare write request to remote host**/
#define ble_cmd_attclient_prepare_write(portHandle,connection,atthandle,offset,data_len,data_data) ble_send_message (portHandle,ble_cmd_attclient_prepare_write_idx,connection,atthandle,offset,data_len,data_data)
/**Send prepare write request to remote host**/
#define ble_cmd_attclient_execute_write(portHandle,connection,commit) ble_send_message (portHandle,ble_cmd_attclient_execute_write_idx,connection,commit)
/**Read multiple attributes from server**/
#define ble_cmd_attclient_read_multiple(portHandle,connection,handles_len,handles_data) ble_send_message (portHandle,ble_cmd_attclient_read_multiple_idx,connection,handles_len,handles_data)
/**Enable encryption on link**/
#define ble_cmd_sm_encrypt_start(portHandle,handle,bonding) ble_send_message (portHandle,ble_cmd_sm_encrypt_start_idx,handle,bonding)
/**Set device to bondable mode**/
#define ble_cmd_sm_set_bondable_mode(portHandle,bondable) ble_send_message (portHandle,ble_cmd_sm_set_bondable_mode_idx,bondable)
/**delete bonding information from ps store**/
#define ble_cmd_sm_delete_bonding(portHandle,handle) ble_send_message (portHandle,ble_cmd_sm_delete_bonding_idx,handle)
/**set pairing requirements**/
#define ble_cmd_sm_set_parameters(portHandle,mitm,min_key_size,io_capabilities) ble_send_message (portHandle,ble_cmd_sm_set_parameters_idx,mitm,min_key_size,io_capabilities)
/**Passkey entered**/
#define ble_cmd_sm_passkey_entry(portHandle,handle,passkey) ble_send_message (portHandle,ble_cmd_sm_passkey_entry_idx,handle,passkey)
/**List all bonded devices**/
#define ble_cmd_sm_get_bonds(portHandle) ble_send_message (portHandle,ble_cmd_sm_get_bonds_idx)
/**
 Set out-of-band encryption data for device
 Device does not allow any other kind of pairing except oob if oob data is set.
 **/
#define ble_cmd_sm_set_oob_data(portHandle,oob_len,oob_data) ble_send_message (portHandle,ble_cmd_sm_set_oob_data_idx,oob_len,oob_data)
/**
 ...
 or being connected.
 **/
#define ble_cmd_sm_whitelist_bonds(portHandle) ble_send_message (portHandle,ble_cmd_sm_whitelist_bonds_idx)
/**Set GAP central/peripheral privacy flags	
 **/
#define ble_cmd_gap_set_privacy_flags(portHandle,peripheral_privacy,central_privacy) ble_send_message (portHandle,ble_cmd_gap_set_privacy_flags_idx,peripheral_privacy,central_privacy)
/**Set discoverable and connectable mode**/
#define ble_cmd_gap_set_mode(portHandle,discover,connect) ble_send_message (portHandle,ble_cmd_gap_set_mode_idx,discover,connect)
/**start or stop discover procedure**/
#define ble_cmd_gap_discover(portHandle,mode) ble_send_message (portHandle,ble_cmd_gap_discover_idx,mode)
/**Direct connection**/
#define ble_cmd_gap_connect_direct(portHandle,address,addr_type,conn_interval_min,conn_interval_max,timeout,latency) ble_send_message (portHandle,ble_cmd_gap_connect_direct_idx,address,addr_type,conn_interval_min,conn_interval_max,timeout,latency)
/**End current GAP procedure**/
#define ble_cmd_gap_end_procedure(portHandle) ble_send_message (portHandle,ble_cmd_gap_end_procedure_idx)
/**Connect to any device on whitelist**/
#define ble_cmd_gap_connect_selective(portHandle,conn_interval_min,conn_interval_max,timeout,latency) ble_send_message (portHandle,ble_cmd_gap_connect_selective_idx,conn_interval_min,conn_interval_max,timeout,latency)
/**Set scan and advertising filtering parameters**/
#define ble_cmd_gap_set_filtering(portHandle,scan_policy,adv_policy,scan_duplicate_filtering) ble_send_message (portHandle,ble_cmd_gap_set_filtering_idx,scan_policy,adv_policy,scan_duplicate_filtering)
/**Set scan parameters**/
#define ble_cmd_gap_set_scan_parameters(portHandle,scan_interval,scan_window,active) ble_send_message (portHandle,ble_cmd_gap_set_scan_parameters_idx,scan_interval,scan_window,active)
/**Set advertising parameters**/
#define ble_cmd_gap_set_adv_parameters(portHandle,adv_interval_min,adv_interval_max,adv_channels) ble_send_message (portHandle,ble_cmd_gap_set_adv_parameters_idx,adv_interval_min,adv_interval_max,adv_channels)
/**Set advertisement or scan response data. Use broadcast mode to advertise data**/
#define ble_cmd_gap_set_adv_data(portHandle,set_scanrsp,adv_data_len,adv_data_data) ble_send_message (portHandle,ble_cmd_gap_set_adv_data_idx,set_scanrsp,adv_data_len,adv_data_data)
 ...
 **/
#define ble_cmd_gap_set_directed_connectable_mode(portHandle,address,addr_type) ble_send_message (portHandle,ble_cmd_gap_set_directed_connectable_mode_idx,address,addr_type)
/**Configure I/O-port interrupts**/
#define ble_cmd_hardware_io_port_config_irq(portHandle,port,enable_bits,falling_edge) ble_send_message (portHandle,ble_cmd_hardware_io_port_config_irq_idx,port,enable_bits,falling_edge)
/**Set soft timer to send events**/
#define ble_cmd_hardware_set_soft_timer(portHandle,time,handle,single_shot) ble_send_message (portHandle,ble_cmd_hardware_set_soft_timer_idx,time,handle,single_shot)
/**Read A/D conversion**/
#define ble_cmd_hardware_adc_read(portHandle,input,decimation,reference_selection) ble_send_message (portHandle,ble_cmd_hardware_adc_read_idx,input,decimation,reference_selection)
/**Configure I/O-port direction**/
#define ble_cmd_hardware_io_port_config_direction(portHandle,port,direction) ble_send_message (portHandle,ble_cmd_hardware_io_port_config_direction_idx,port,direction)
/**Configure I/O-port function**/
#define ble_cmd_hardware_io_port_config_function(portHandle,port,function) ble_send_message (portHandle,ble_cmd_hardware_io_port_config_function_idx,port,function)
/**Configure I/O-port pull-up/pull-down**/
#define ble_cmd_hardware_io_port_config_pull(portHandle,port,tristate_mask,pull_up) ble_send_message (portHandle,ble_cmd_hardware_io_port_config_pull_idx,port,tristate_mask,pull_up)
/**Write I/O-port**/
#define ble_cmd_hardware_io_port_write(portHandle,port,mask,data) ble_send_message (portHandle,ble_cmd_hardware_io_port_write_idx,port,mask,data)
/**Read I/O-port**/
#define ble_cmd_hardware_io_port_read(portHandle,port,mask) ble_send_message (portHandle,ble_cmd_hardware_io_port_read_idx,port,mask)
/**Configure SPI**/
#define ble_cmd_hardware_spi_config(portHandle,channel,polarity,phase,bit_order,baud_e,baud_m) ble_send_message (portHandle,ble_cmd_hardware_spi_config_idx,channel,polarity,phase,bit_order,baud_e,baud_m)
/**Transfer SPI data**/
#define ble_cmd_hardware_spi_transfer(portHandle,channel,data_len,data_data) ble_send_message (portHandle,ble_cmd_hardware_spi_transfer_idx,channel,data_len,data_data)
/**Read data from I2C bus using bit-bang in cc2540. I2C clk is in P1.7 data in P1.6. Pull-up must be enabled on pins**/
#define ble_cmd_hardware_i2c_read(portHandle,address,stop,length) ble_send_message (portHandle,ble_cmd_hardware_i2c_read_idx,address,stop,length)
/**Write data to I2C bus using bit-bang in cc2540. I2C clk is in P1.7 data in P1.6. Pull-up must be enabled on pins**/
#define ble_cmd_hardware_i2c_write(portHandle,address,stop,data_len,data_data) ble_send_message (portHandle,ble_cmd_hardware_i2c_write_idx,address,stop,data_len,data_data)
/**Set TX Power**/
#define ble_cmd_hardware_set_txpower(portHandle,power) ble_send_message (portHandle,ble_cmd_hardware_set_txpower_idx,power)
/**Set comparator for timer channel**/
#define ble_cmd_hardware_timer_comparator(portHandle,timer,channel,mode,comparator_value) ble_send_message (portHandle,ble_cmd_hardware_timer_comparator_idx,timer,channel,mode,comparator_value)
/**Enable I/O-port interrupts**/
#define ble_cmd_hardware_io_port_irq_enable(portHandle,port,enable_bits) ble_send_message (portHandle,ble_cmd_hardware_io_port_irq_enable_idx,port,enable_bits)
/**Set I/O-port interrupt direction**/
#define ble_cmd_hardware_io_port_irq_direction(portHandle,port,falling_edge) ble_send_message (portHandle,ble_cmd_hardware_io_port_irq_direction_idx,port,falling_edge)
/**Enable or disable analog comparator**/
#define ble_cmd_hardware_analog_comparator_enable(portHandle,enable) ble_send_message (portHandle,ble_cmd_hardware_analog_comparator_enable_idx,enable)
/**Read analog comparator output**/
#define ble_cmd_hardware_analog_comparator_read(portHandle) ble_send_message (portHandle,ble_cmd_hardware_analog_comparator_read_idx)
/**Configure analog comparator interrupt**/
#define ble_cmd_hardware_analog_comparator_config_irq(portHandle,enabled) ble_send_message (portHandle,ble_cmd_hardware_analog_comparator_config_irq_idx,enabled)
/**Set RX Sensitivity**/
#define ble_cmd_hardware_set_rxgain(portHandle,gain) ble_send_message (portHandle,ble_cmd_hardware_set_rxgain_idx,gain)
/**Enable or disable USB**/
#define ble_cmd_hardware_usb_enable(portHandle,enable) ble_send_message (portHandle,ble_cmd_hardware_usb_enable_idx,enable)
/**Start packet transmission, send one packet at every 625us**/
#define ble_cmd_test_phy_tx(portHandle,channel,length,type) ble_send_message (portHandle,ble_cmd_test_phy_tx_idx,channel,length,type)
/**Start receive test**/
#define ble_cmd_test_phy_rx(portHandle,channel) ble_send_message (portHandle,ble_cmd_test_phy_rx_idx,channel)
/**End test, and report received packets**/
#define ble_cmd_test_phy_end(portHandle) ble_send_message (portHandle,ble_cmd_test_phy_end_idx)
/**Reset test**/
#define ble_cmd_test_phy_reset(portHandle) ble_send_message (portHandle,ble_cmd_test_phy_reset_idx)
/**Get current channel quality map**/
#define ble_cmd_test_get_channel_map(portHandle) ble_send_message (portHandle,ble_cmd_test_get_channel_map_idx)
/**Debug command**/
#define ble_cmd_test_debug(portHandle,input_len,input_data) ble_send_message (portHandle,ble_cmd_test_debug_idx,input_len,input_data)
/**Set channel quality measurement mode**/
#define ble_cmd_test_channel_mode(portHandle,mode) ble_send_message (portHandle,ble_cmd_test_channel_mode_idx,mode)
/**Reset system**/
#define ble_cmd_dfu_reset(portHandle,dfu) ble_send_message (portHandle,ble_cmd_dfu_reset_idx,dfu)
/**set address for flashing**/
#define ble_cmd_dfu_flash_set_address(portHandle,address) ble_send_message (portHandle,ble_cmd_dfu_flash_set_address_idx,address)
/**Upload binary for flashing. Address will be updated automatically.**/
#define ble_cmd_dfu_flash_upload(portHandle,data_len,data_data) ble_send_message (portHandle,ble_cmd_dfu_flash_upload_idx,data_len,data_data)
/**Uploading is finished.**/
#define ble_cmd_dfu_flash_upload_finish(portHandle) ble_send_message (portHandle,ble_cmd_dfu_flash_upload_finish_idx)

/*
 *Function prototypes of callbacks in commands.c. Implemented functions
 *in response to events and receives.
 */
/**Reset device**/
...
/**Get Device info**/
void ble_rsp_system_get_info(void* bled_instance,const struct ble_msg_system_get_info_rsp_t *msg);
...
/**End current GAP procedure**/
void ble_rsp_gap_end_procedure(void* bled_instance,
...
/**Set scan parameters**/
void ble_rsp_gap_set_scan_parameters(void* bled_instance,
...
/**Scan Response**/
void ble_evt_gap_scan_response(void* bled_instance,
